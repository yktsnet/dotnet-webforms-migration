...
export const api = {
  ...
  getCurrentAttendance: (): Promise<import('./types').CurrentAttendance[]> =>
    fetch(`${BASE}/attendances/current`).then(r => r.json()),
  ...
  correctAttendance: (
    id: number,
    req: { clockIn: string; clockOut: string; breakMinutes: number },
    token: string
  ) =>
