...
[dotnet-winforms-migration](https://github.com/kyamakawa-widget/dotnet-winforms-migration)（WinForms 移行）の姉妹リポ。WebForms 固有の問題（AutoPostBack・ViewState・Page_Load 集中）の解体と再構成に加え、**WebForms では構造的に実現不可能だったリアルタイム機能**の追加まで扱う。
...
**After Demo:** *(Cloudflare Tunnel — 稼働時のみ)*  
...
`AttendanceCalculator` を `AttendanceService` から独立させ、DB 接続なしで計算ロジック単体をテスト可能にしている。休憩時間（デフォルト 60 分・管理者が ±分で調整）の控除、端数処理（社員ごとの丸め単位）、残業割増（法定超過 × 1.25）が対象。
...
| 機能 | 実装 | 内容 |
|---|---|---|
| リアルタイム出勤ボード | SignalR (`AttendanceHub`) | 打刻のたびに全クライアントへ Push。ページリロード不要。 |
| 36 協定アラート | SignalR + 閾値チェック | 退勤打刻時に月次残業が閾値に達していたら管理者グループへ Push |
| 未退勤アラート | `IHostedService` + SignalR | 30 分ごとに検査。平均退勤時刻 +1h を超えて未退勤の社員を管理者へ Push |
| 平均退勤プロファイル | `IHostedService`（日次） | 直近 30 日の退勤時刻平均を `employee_profiles` に保存。修正フォームのデフォルト値にも使用。 |
...
6. **CI/CD のパイプライン化 (GitHub Actions)**: push ごとにビルド・テストを自動実行。PostgreSQL サービスコンテナで統合テストも CI 上で完結。
7. **構造分離後の拡張性**: 責務が分離された構造では、WebForms では実装不可能だった Push 型機能を後から追加できる。SignalR の統合がその実証。
...
### Before デモ
Cloudflare Pages により常時稼働。URL 固定。

### After デモ

[dotnet-winforms-migration](https://github.com/kyamakawa-widget/dotnet-winforms-migration)（WinForms After）と本リポ（WebForms After）は、**同一の Cloudflare Tunnel・同一 URL** を共用している。Tunnel は常時稼働のまま、背後の Docker Compose スタックを排他的に切り替えることで、URL を変えずに 2 つの After デモを提示できる。

```mermaid
graph LR
    User["ブラウザ"]
    Pages["Cloudflare Pages\n（Before デモ・常時稼働）"]
    Tunnel["Cloudflare Tunnel\n同一 URL / localhost:5154"]
    subgraph SERVER["オンプレサーバー（NixOS）"]
        SVC1["winforms-migration\nDocker Compose"]
        SVC2["webforms-migration\nDocker Compose"]
        DB[("PostgreSQL")]
    end
...
```bash
# 1. サーバー上で Docker を有効化（/etc/nixos/configuration.nix に追加）
#   virtualisation.docker.enable = true;
#   users.users.<username>.extraGroups = [ "docker" ];
#   → sudo nixos-rebuild switch && 再ログイン

# 2. サーバー上でセットアップ
./infrastructure/setup.sh

# 3. Mac からデプロイ
cp .env.example .env  # DEPLOY_HOST / DEPLOY_USER を記入
...
```bash
./infrastructure/switch-demo.sh webforms   # WebForms を起動
./infrastructure/switch-demo.sh winforms   # WinForms を起動
```

---

## 8. dotnet-winforms-migration との対比

| | [dotnet-winforms-migration](https://github.com/kyamakawa-widget/dotnet-winforms-migration) | dotnet-webforms-migration（本リポ） |
|---|---|---|
...
│   ├── db-init.sh                        # 手動 DB スキーマ適用（任意）
│   ├── deploy.sh                         # ソース転送・docker compose up --build
│   ├── setup.sh                          # サーバー初回セットアップ（Docker 確認・ディレクトリ作成）
│   └── switch-demo.sh                    # WinForms ↔ WebForms 排他切替（サーバー上実行）
├── legacy/
